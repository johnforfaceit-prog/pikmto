...
Верни ТОЛЬКО JSON без пояснений и markdown:
{
  ...
  "EXECUTOR": "полное наименование ИСПОЛНИТЕЛЯ — из блока «Поставщик (Исполнитель)» счёта, «Продавец» в счёте-фактуре/УПД или «Исполнитель» на экране ЕИС. НЕ заказчик (НЕ МКУ «ЦОД»/«Центр обеспечения деятельности...»)",
  "ADDRESS": "место нахождения (адрес) исполнителя — из блока «Поставщик (Исполнитель)» счёта: индекс, область, город, улица, дом",
  "SUBJECT": "предмет контракта одной строкой — из «Предмет контракта» на экране ЕИС (например 'Оказание образовательных услуг в 2026 году')",
...
— Если поле не найдено — пустая строка. Даты в формате ДД.ММ.ГГГГ. Суммы числом без пробелов и символа валюты.`;
  ...
  // Реквизиты исполнителя, предмет и контракт — из листов по счёту (надёжный источник).
  // В режиме «Новый контракт» данные листов имеют приоритет (контракт-документ парсится хуже);
  // в режиме «Действующий» — только дозаполняем пустые поля, не затирая данные прошлого заключения.
  const preferInv = (S.mode === 'new');
  ['EXECUTOR','ADDRESS','SUBJECT','CONTRACT_NUM','CONTRACT_DATE'].forEach(k => {
    if (parsed[k] && (preferInv || !S.f[k])) S.f[k] = String(parsed[k]).trim();
  });
